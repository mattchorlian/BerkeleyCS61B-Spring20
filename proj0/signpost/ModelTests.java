...
public class ModelTests {
    ...
    public void arrowTest1() {
        ...
        for (int x = 0; x < model.width(); x += 1) {
            for (int y = 0; y < model.height(); y += 1) {
                assertEquals(tr(ARROWS1)[x][y], model.arrowDirection(x, y));
            }
        }
        Model model2 = new Model(tr(SOLN2));
        for (int x = 0; x < model2.width(); x += 1) {
            for (int y = 0; y < model2.height(); y += 1) {
                assertEquals(tr(ARROWS2)[x][y], model2.arrowDirection(x, y));
            }
        }
