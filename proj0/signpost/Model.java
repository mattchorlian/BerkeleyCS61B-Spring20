...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                boolean fixednum = false; int seqnum = 0; int group = -1;
                if (solution[x][y] == 1 || solution[x][y] == last) {
                    fixednum = true; seqnum = solution[x][y]; group = 0;
                }
                int direct = arrowDirection(x, y);
                _board[x][y] =
                        new Sq(x, y, seqnum, fixednum, direct, group);
                _allSquares.add(_board[x][y]);
                _solnNumToPlace[_solution[x][y] - 1] = pl(x, y);
            }
        }
        ...
        for (Sq sq: _allSquares) {
            ...
            sq._predecessors = allSuccessors(sq.pl, 0);
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                ...
                boolean fixed = temp._hasFixedNum;
                int seqnum = temp._sequenceNum;
                _board[i][j] =
                        new Sq(i, j, seqnum, fixed, temp._dir, temp._group);
        ...
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                Sq temp = model.get(i, j);
                Sq sqsucc = temp._successor;
                Sq sqpred = temp._predecessor;
                Sq head = temp._head;
                if (temp._successor != null) {
                    _board[i][j]._successor = _board[sqsucc.x][sqsucc.y];
                }
                if (temp._predecessor != null) {
                    _board[i][j]._predecessor = _board[sqpred.x][sqpred.y];
                }
                if (temp._head != null) {
    ...
    boolean autoconnect() {
        ...
        for (int i = 0; i < _allSquares.size(); i++) {
            ...
            for (int j = 0; j < _allSquares.size(); j++) {
                ...
                int sq1sn = sq1._sequenceNum;
                int sq2sn = sq2._sequenceNum;
                if (sq1.connectable(sq2) && sq1sn == sq2sn - 1) {
                    sq1.connect(sq2);
                    changed = true;
                }
            }
    ...
    void solve() {
        Sq[] allsquares = new Sq[_width * _height];
        for (int i = 0; i < _width; i++) {
            for (int j = 0; j < _height; j++) {
                _board[i][j]._sequenceNum = _solution[i][j];
                allsquares[_solution[i][j] - 1] = _board[i][j];
            }
        }
        allsquares[0]._successor = allsquares[1];
        for (int i = 1; i < _width * _height - 1; i++) {
            allsquares[i]._predecessor = allsquares[i - 1];
            allsquares[i]._successor = allsquares[i + 1];
        }
        int last = _width * _height;
        allsquares[last - 1]._predecessor = allsquares[last - 2];
        _unconnected = 0;
    }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (_group != 0 && s1._group != 0) {
                combinedgroups = joinGroups(_group, s1._group);
            } else if (_group == 0) {
            ...
            if (_sequenceNum == 0 && s1._sequenceNum == 0) {
                ...
                if (this._sequenceNum != 0) {
                    s1._sequenceNum = _sequenceNum + 1;
                } else {
                    _sequenceNum = s1._sequenceNum - 1;
                }
                Sq successor = _successor;
                int sequencetracker = _sequenceNum + 1;
                while (successor != null) {
                    successor._sequenceNum = sequencetracker;
                    successor._head = _head;
                    successor._group = _group;
                    successor = successor._successor;
                    sequencetracker += 1;
                }
                Sq predecessor = _predecessor;
                sequencetracker = _sequenceNum - 1;
                while (predecessor != null) {
                    predecessor._sequenceNum = sequencetracker;
                    predecessor._head = _head;
                    predecessor._group = _group;
        ...
        void helper() {
            if (_predecessor != null) {
                _group = newGroup(); Sq loopy = _predecessor;
                while (loopy != null) {
                    loopy._group = _group; loopy = loopy._predecessor;
                }
            } else {
                _group = -1;
            }
            Sq iter = this;
            while (iter != null) {
                iter._sequenceNum = 0; iter = iter._predecessor;
            }
        }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (next._successor == null && _predecessor == null) {
                    releaseGroup(_group); releaseGroup(this._group);
                ...
                Sq iter = this; boolean bool = false;
                while (iter != null) {
                    if (iter.hasFixedNum()) {
                        bool = true; break;
                    }
                    iter = iter._predecessor;
                }
                if (!bool) {
                    helper();
                }
                iter = next; bool = false;
                while (iter != null) {
                    if (iter.hasFixedNum()) {
                        bool = true; break;
                    }
                    iter = iter._successor;
                } if (!bool) {
                    if (next._successor != null) {
                        next._group = newGroup(); Sq itr = next;
                        while (itr != null) {
                            itr._group = next._group; itr = itr._successor;
                        }
                    } else {
                        next._group = -1;
                    }
                    iter = next;
                    while (iter != null) {
                        iter._sequenceNum = 0; iter = iter._successor;
                    }
                }
            }
            Sq loopy = next;
            while (loopy != null) {
                loopy._head = next; loopy = loopy._successor;
            }
        }
