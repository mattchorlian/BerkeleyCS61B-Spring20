...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        PlaceList successorlist = model.allSuccessors(start.pl, dir);
        if (successorlist.size() == 1) {
            Place succ = successorlist.get(0);
            return model.get(succ);
        }
        if (successorlist.size() > 1) {
            Sq connectable = null;
            boolean ambiguous = false;
            for (int i = 0; i < successorlist.size(); i++) {
                Place succ = successorlist.get(i);
                Sq successor = model.get(succ);
                int nextNum = successor.sequenceNum();
                int startnum = start.sequenceNum();
                if (start.connectable(successor)
                        && (nextNum != 1)
                        && (startnum == 0
                        || nextNum == 0
                        || nextNum - 1 == startnum)) {
                    if (startnum == nextNum - 1 && startnum != 0) {
                        return successor;
                    }
                    if (connectable != null) {
                        ambiguous = true;
                    }
                    connectable = successor;
                }
            }
            if (ambiguous) {
                return null;
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        PlaceList predecessorlist = model.allSuccessors(end.pl, 0);
        if (predecessorlist.size() == 1) {
            Place pred = predecessorlist.get(0);
            return model.get(pred);
        }
        if (predecessorlist.size() > 1) {
            boolean ambiguous = false;
            Sq connectable = null;
            for (int i = 0; i < predecessorlist.size(); i++) {
                Place pred = predecessorlist.get(i);
                Sq predecessor = model.get(pred);
                int prevNum = predecessor.sequenceNum();
                int endnum = end.sequenceNum();
                if (predecessor.connectable(end)
                        && (prevNum != model.height() * model.width())
                        && (endnum == 0
                        || prevNum == 0
                        || prevNum + 1 == endnum)) {
                    if (endnum == prevNum + 1 && endnum != 0) {
                        return predecessor;
                    }
                    if (connectable != null) {
                        ambiguous = true;
                    }
                    connectable = predecessor;
                }
